...
class UI {
  ...
  clearCart() {
    const inputFields = document.querySelectorAll('.input_required'); // Replace with your actual input class

    // Check if any of the input fields are empty
    const isAnyInputEmpty = Array.from(inputFields).some(
      (input) => input.value.trim() === ''
    );

    if (isAnyInputEmpty) {
      alert('Please fill in all the required fields.');
      return; // Exit the function without clearing the cart
    }
